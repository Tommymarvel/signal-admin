...
const TransactionTable = () => {
  ...
  const toggleSelectAll = () => {
    if (selectAll) {
      ...
      setSelectedTrx(currentTrx.map((trx) => String(trx.id)));
    }
  ...
  useEffect(() => {
    const getAllTrx = async () => {
      try {
        ...
        console.log('Wallet transactions API response:', res);
      ...
      <main className=" bg-white rounded-lg">
        ...
        <div className="flex justify-between items-center mt-4">
          <p className="text-sm text-gray-600">
            {indexOfFirstTrx + 1}-{Math.min(indexOfLastTrx, filteredTrx.length)}{' '}
            of {filteredTrx.length} transactions
          </p>
          <div className="flex space-x-2">
            {[...Array(Math.ceil(trxs?.length / trxsPerPage)).keys()].map(
              (number) => (
                <button
                  key={number + 1}
